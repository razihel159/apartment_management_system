...
import 'landlord_reports_screen.dart'; // 1. SIGURADUHIN NA IMPORTED ITO
...
class _LandlordDashboardState extends State<LandlordDashboard> {
  ...
  Map<String, dynamic> stats = {"totalRooms": "0", "occupiedRooms": "0", "vacantRooms": "0", "totalCollected": "0"};
  ...
  Widget build(BuildContext context) {
    // 2. DAGDAGAN ANG PAGES LIST
    final List<Widget> pages = [
      _buildDashboardOverview(), 
      const RoomManagementScreen(),
      const TenantManagementScreen(),
      const PaymentScreen(),
      const LandlordReportsScreen(), // Index 4: Bagong Reports Screen
      const SettingsScreen(),        // Index 5: Nilipat ang Settings
    ];
      ...
      body: Row(
        children: [
          Container(
            ...
            child: Column(
              children: [
                const DrawerHeader(child: Center(child: Text("APARTMENT ADMIN", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 20)))),
                ...
                _sidebarTile(Icons.report_problem, "Tenant Reports", 4), // 3. DAGDAG SA SIDEBAR
                _sidebarTile(Icons.settings, "Settings", 5),             // 4. UPDATE INDEX NG SETTINGS
              ],
            ),
          ),
          Expanded(
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 20,
          children: [
            _actionButton("Add Room", Icons.add, 1),
            _actionButton("Register Tenant", Icons.person_add, 2),
            _actionButton("View Reports", Icons.warning_amber, 4), // 5. DAGDAG SA QUICK ACTIONS
          ],
        )
      ],
    );
  ...
  Widget _actionButton(String label, IconData icon, int targetIndex) {
    return ElevatedButton.icon(
      ...
      style: ElevatedButton.styleFrom(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30), side: BorderSide(color: Colors.purple.withOpacity(0.2)))
      ),
    );
  }
    ...
    return Expanded(
      child: Card(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border(left: BorderSide(color: color, width: 6))),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
